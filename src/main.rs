...
fn run(api: HidApi) -> QKResult<()> {
    ...
    match QKDevice::open(api, ConnectionMode::Auto) {
        Ok(dev) => {
            ...
            dev.set_wheel_speed(WheelSpeed::Slowest)?;
            ...
            loop {
                match dev.read() {
                    Ok(ev) => match ev {
                        Event::Wheel { direction: WheelDirection::Left } => Ok(enigo.key_sequence_parse("{+META}{F11}{-META}")),
                        Event::Wheel { direction: WheelDirection::Right } => Ok(enigo.key_sequence_parse("{+META}{F12}{-META}")),
                        ...
                        Event::Button { state: ButtonState { button_6: true, .. } } => Ok(enigo.mouse_move_relative(10, 10)),
                        Event::Button { state: ButtonState { button_7: true, .. } } => Ok(enigo.key_sequence_parse("{+CTRL}{F8}{-CTRL}")),
